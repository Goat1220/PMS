...
public class YearendTaxSimCommandService {
    ...
    현재 DB의 yrt_item 테이블과 동일하게 맞춤
    DB 수정 없이 실행 가능하게 조정
    ----------------------------------------------------- */
    private void seedScenarioItems(Long yrtId) {
        // [세액공제] 자녀세액공제 / 【税額控除】子女税額控除
        mapper.insertItem(yrtId, "세액공제", "자녀세액공제", 500_000L, 500_000L);

        // [세액공제] 보험료세액공제 / 【税額控除】保険料税額控除
        mapper.insertItem(yrtId, "세액공제", "보험료세액공제", 300_000L, 300_000L);

        // [세액공제] 교육비세액공제 / 【税額控除】教育費税額控除
        mapper.insertItem(yrtId, "세액공제", "교육비세액공제", 400_000L, 400_000L);

        // [세액공제] 기부금세액공제 / 【税額控除】寄付金税額控除
        mapper.insertItem(yrtId, "세액공제", "기부금세액공제", 200_000L, 200_000L);
    }
}