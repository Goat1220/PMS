...
<div class="wrap">
  ...
  <!-- 타이틀 -->
  <div class="titlebar"><div id="popupTitle">부서</div></div>
 ...
 <div class="searchbar">
    <!-- 사용/미사용 드롭다운 -->
    <span class="muted" style="margin-left:12px;">사용여부</span>
    <select id="selUse" title="사용여부">
...
<script>
  window.POPUP_CONFIG = {
    ...
    columns: [
      ...
      { key:'deptCode', name:'부서코드', width:80 },
      { key:'deptName', name:'부서명' },
      { key:'startDate',name:'시작일',  width:100, format:'date' },
  ...
  // 드롭다운 변경 시 즉시 재조회
  document.getElementById('selUse').addEventListener('change', function () {
    if (window.POPUP && POPUP.search) POPUP.search(); // 없으면 아래 한 줄로
    else document.getElementById('btnSearch').click();
  });
